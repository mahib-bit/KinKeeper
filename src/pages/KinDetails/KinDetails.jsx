...
const KinDetails = () => {
    ...
    const { name, status, tags, picture, bio, days_since_contact, goal, next_due_date, email } = detail;

    const getStatusBadge = (status) => {
        if (status === 'on_track') {
            return <span className="badge bg-green-600 text-white border-none font-medium px-3 py-1">On Track</span>;
        }
        if (status === 'overdue') {
            return <span className="badge bg-red-500 text-white border-none font-medium px-3 py-1">Overdue</span>;
        }
        return <span className="badge badge-ghost font-medium px-3 py-1">{status}</span>;
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-8 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-200 text-center flex flex-col items-center justify-center">
                            <span className="text-3xl font-bold text-green-800">{days_since_contact}</span>
                            <span className="text-xs font-medium text-base-content/60 mt-2">Days Since Contact</span>
                        </div>
                        <div className="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-200 text-center flex flex-col items-center justify-center">
                            <span className="text-3xl font-bold text-green-800">{goal}</span>
                            <span className="text-xs font-medium text-base-content/60 mt-2">Goal (Days)</span>
                        </div>
                        <div className="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-200 text-center flex flex-col items-center justify-center">
                            <span className="text-xl sm:text-2xl font-bold text-green-800">{next_due_date || 'N/A'}</span>
                            <span className="text-xs font-medium text-base-content/60 mt-2">Next Due</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-200 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-base-content/80 mt-2">
                                Connect every <span className="font-bold text-base-content">{goal} days</span>
                            </p>
                        </div>
                        <button className="btn btn-sm bg-base-200 hover:bg-base-300 border-none font-medium px-4 rounded-lg">
                            Edit
                        </button>
                    </div>
                    ...
                    <div className="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-200">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <button className="btn bg-base-200/60 hover:bg-base-200 border-none h-24 flex flex-col gap-2 rounded-xl text-base-content font-medium">
                                <Phone className="w-5 h-5 text-base-content" />
                                Call
                            </button>
                            <button className="btn bg-base-200/60 hover:bg-base-200 border-none h-24 flex flex-col gap-2 rounded-xl text-base-content font-medium">
                            ...
                            <button className="btn bg-base-200/60 hover:bg-base-200 border-none h-24 flex flex-col gap-2 rounded-xl text-base-content font-medium">
                                <Video className="w-5 h-5 text-base-content" />
                                Video
                            </button>
                        </div>
                    </div>
